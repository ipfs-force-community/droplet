...
type travelCallbackTemplate func(unmarshaler cbg.CBORUnmarshaler) (exitLoop bool, err error)
...
func checkCallbackAndGetParamType(i interface{}) (reflect.Type, error) {
	...
	if t.NumOut() != 2 {
		return nil, xerrors.Errorf("callback returns value number : %d != %d", t.NumOut(), 2)
	}
	in := t.In(0)
	...
	if t.Out(0).Kind() != reflect.Bool {
		return nil, xerrors.Errorf("1st return value must be a boolean")
	}

	if !t.Out(1).Implements(reflect.TypeOf((*error)(nil)).Elem()) {
		return nil, xerrors.Errorf("2nd return value must be an error interface")
	}
	return in.Elem(), nil
}
...
func travelDeals(ds datastore.Batching, callback interface{}) error {
	...
	for res := range result.Next() {
		...
		if stop := rets[0].Interface().(bool); stop {
			return nil
		}
	}
	return nil
