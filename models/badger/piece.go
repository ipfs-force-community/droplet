...
func NewBadgerPieceRepo(pieceDs repo.PieceInfoDS, cidInfoDs repo.CIDInfoDS) (repo.IPieceRepo, error) {
	...
	pieces, migratePieces := statestore.NewVersionedStateStore(pieceDs, pieceInfoMigrations, "1")

	cidInfoMigrations, err := migrations.CIDInfoMigrations.Build()
	if err != nil {
		return nil, err
	}
	cidInfos, migrateCidInfos := statestore.NewVersionedStateStore(cidInfoDs, cidInfoMigrations, "1")
	return &pieceStore{
