...
var listStorageAsksCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		for _, sask := range asks {
			...
			_, _ = fmt.Fprintf(w, "%v\t%v\t%v\t%v\t%v\t%d\t%v\t%d\n", ask.Miner, types.FIL(ask.Price),
				types.FIL(ask.VerifiedPrice), types.SizeStr(types.NewInt(uint64(ask.MinPieceSize))),
