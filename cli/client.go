...
var ClientCmds = []*cli.Command{
	...
	WithCategory("util", clientAddFunds),
	WithCategory("util", clientWithdraw),
...
var clientImportCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		api, closer, err := NewMarketClientNode(cctx)
		if err != nil {
			return err
		}
		defer closer()
...
var clientRetrieveCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		mapi, closer, err := NewMarketClientNode(cctx)
		if err != nil {
			return err
		}
		defer closer()
...
var clientAddFunds = &cli.Command{
	Name:      "add-funds",
...
var clientWithdraw = &cli.Command{
	Name:      "withdraw",
	...
	Action: func(cctx *cli.Context) error {
		...
		if watch {
			...
			for {
				...
				OutputDataTransferChannels(tm.Screen, channels, verbose, completed, false, showFailed)
		...
		OutputDataTransferChannels(os.Stdout, channels, verbose, completed, false, showFailed)
		return nil
	},
}
