...
The minimum value is 518400 (6 months).`,
	...
	Action: func(cctx *cli.Context) error {
		...
		minerInfo, err := fapi.StateMinerInfo(ctx, p.miner[0], types.EmptyTSK)
		if err != nil {
			return err
		}
		...
		addrInfo := peer.AddrInfo{ID: *minerInfo.PeerId, Addrs: addrs}

		if err := h.Connect(ctx, addrInfo); err != nil {
			return fmt.Errorf("failed to connect to peer %s: %w", minerInfo.PeerId, err)
		}
		x, err := h.Peerstore().FirstSupportedProtocol(*minerInfo.PeerId, types2.DealProtocolv121ID)
		if err != nil {
			return fmt.Errorf("getting protocols for peer %s: %w", minerInfo.PeerId, err)
		}
