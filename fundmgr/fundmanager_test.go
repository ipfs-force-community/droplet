...
func TestFundManagerReserveByWallet(t *testing.T) {
	...
	walletAddrA, err := s.wllt.NewAddress(context.Background(), address.SECP256K1)
	require.NoError(t, err)
	walletAddrB, err := s.wllt.NewAddress(context.Background(), address.SECP256K1)
	require.NoError(t, err)
...
func TestFundManagerRestart(t *testing.T) {
	...
	fmAfter := newFundManager(mockApiAfter, s.fundRepo)
	err = fmAfter.Start(context.Background())
	require.NoError(t, err)
...
func setup(t *testing.T) *scaffold {
	...
	walletAddr, err := wllt.NewAddress(context.Background(), address.SECP256K1)
	if err != nil {
		t.Fatal(err)
	}
	...
	fundRepo := badger.NewFundRepo(ds_sync.MutexWrap(ds.NewMapDatastore()))
	fm := newFundManager(mockApi, fundRepo)
	return &scaffold{
		ctx:        ctx,
		wllt:       wllt,
...
func (mapi *mockFundManagerAPI) PushMessage(ctx context.Context, message *types.Message, spec *types.MessageSendSpec) (cid.Cid, error) {
	mapi.lk.Lock()
...
func (mapi *mockFundManagerAPI) WaitMsg(ctx context.Context, c cid.Cid, confidence uint64, limit abi.ChainEpoch, allowReplaced bool) (*types.MsgLookup, error) {
	res := &types.MsgLookup{
		Message: c,
