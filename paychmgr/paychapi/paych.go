...
func (a *PaychAPI) PaychGet(ctx context.Context, from, to address.Address, amt types.BigInt) (*markettype.ChannelInfo, error) {
	ch, mcid, err := a.PaychMgr.GetPaych(ctx, from, to, amt)
	if err != nil {
		return nil, err
	}

	return &markettype.ChannelInfo{
		Channel:      ch,
		WaitSentinel: mcid,
	}, nil
}

func (a *PaychAPI) PaychAvailableFunds(ctx context.Context, ch address.Address) (*markettype.ChannelAvailableFunds, error) {
	return a.PaychMgr.AvailableFunds(ch)
}

func (a *PaychAPI) PaychAvailableFundsByFromTo(ctx context.Context, from, to address.Address) (*markettype.ChannelAvailableFunds, error) {
...
func (a *PaychAPI) PaychNewPayment(ctx context.Context, from, to address.Address, vouchers []markettype.VoucherSpec) (*markettype.PaymentInfo, error) {
...
func (a *PaychAPI) PaychStatus(ctx context.Context, pch address.Address) (*markettype.PaychStatus, error) {
	ci, err := a.PaychMgr.GetChannelInfo(pch)
	if err != nil {
		return nil, err
	}
	return &markettype.PaychStatus{
		ControlAddr: ci.Control,
		Direction:   markettype.PCHDir(ci.Direction),
	}, nil
}
...
func (a *PaychAPI) PaychVoucherCreate(ctx context.Context, pch address.Address, amt types.BigInt, lane uint64) (*markettype.VoucherCreateResult, error) {
