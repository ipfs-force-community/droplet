...
func (pm *Manager) CreateVoucher(ctx context.Context, ch address.Address, voucher types2.SignedVoucher) (*VoucherCreateResult, error) {
	ca, err := pm.accessorByAddress(ctx, ch)
	if err != nil {
		return nil, err
	}
...
func (pm *Manager) CheckVoucherValid(ctx context.Context, ch address.Address, sv *types2.SignedVoucher) error {
...
func (pm *Manager) CheckVoucherSpendable(ctx context.Context, ch address.Address, sv *types2.SignedVoucher, secret []byte, proof []byte) (bool, error) {
	if len(proof) > 0 {
...
func (pm *Manager) AddVoucherOutbound(ctx context.Context, ch address.Address, sv *types2.SignedVoucher, proof []byte, minDelta big.Int) (big.Int, error) {
	if len(proof) > 0 {
...
// If the channel is not in the store, fetches the channel from state (and checks that
// the channel To address is owned by the wallet).
func (pm *Manager) AddVoucherInbound(ctx context.Context, ch address.Address, sv *types2.SignedVoucher, proof []byte, minDelta big.Int) (big.Int, error) {
	if len(proof) > 0 {
		return big.NewInt(0), errProofNotSupported
	}
	// Get an accessor for the channel, creating it from state if necessary
...
func (pm *Manager) SubmitVoucher(ctx context.Context, ch address.Address, sv *types2.SignedVoucher, secret []byte, proof []byte) (cid.Cid, error) {
	if len(proof) > 0 {
