...
func NewManager(ctx context.Context, shutdown func(), pchstore *Store, api PaychAPI) *Manager {
...
func (pm *Manager) AvailableFunds(ch address.Address) (*markettype.ChannelAvailableFunds, error) {
...
func (pm *Manager) AvailableFundsByFromTo(from address.Address, to address.Address) (*markettype.ChannelAvailableFunds, error) {
	...
	if err == ErrChannelNotTracked {
		...
		// an error.
		return &markettype.ChannelAvailableFunds{
			Channel:             nil,
			From:                from,
			To:                  to,
...
func (pm *Manager) CreateVoucher(ctx context.Context, ch address.Address, voucher paych.SignedVoucher) (*markettype.VoucherCreateResult, error) {
	ca, err := pm.accessorByAddress(ch)
	if err != nil {
		return nil, err
	}
