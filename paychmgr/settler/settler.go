...
import (
	...
	"github.com/filecoin-project/venus-market/paychmgr/paychapi"
)
...
type settlerAPI interface {
	...
	PaychStatus(context.Context, address.Address) (*markettype.PaychStatus, error)
...
func SettlePaymentChannels(mctx metrics.MetricsCtx, lc fx.Lifecycle, papi API) error {
	ctx := metrics.LifecycleCtx(mctx, lc)
	lc.Append(fx.Hook{
		OnStart: func(context.Context) error {
			pcs := newPaymentChannelSettler(ctx, &papi)
			ev := events.NewEvents(ctx, papi)
			return ev.Called(pcs.check, pcs.messageHandler, pcs.revertHandler, int(constants.MessageConfidence+1), events.NoTimeout, pcs.matcher)
		},
	})
	return nil
}
...
func (pcs *paymentChannelSettler) messageHandler(msg *types.Message, rec *types.MessageReceipt, ts *types.TipSet, curH abi.ChainEpoch) (more bool, err error) {
	...
	for _, voucher := range bestByLane {
		...
		go func(voucher *paych.SignedVoucher, submitMessageCID cid.Cid) {
			defer wg.Done()
			msgLookup, err := pcs.api.StateWaitMsg(pcs.ctx, submitMessageCID, uint64(constants.MessageConfidence), constants.LookbackNoLimit, true)
			if err != nil {
...
func (pcs *paymentChannelSettler) matcher(msg *types.Message) (matched bool, err error) {
	...
	for _, addr := range trackedAddresses {
		if msg.To == addr {
			...
			if status.Direction == markettype.PCHInbound {
				return true, nil
			}
		}
	}
