...
func (ca *apiWrapper) diffPreCommits(ctx context.Context, actor address.Address, pre, cur types.TipSetKey) (*miner.PreCommitChanges, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("getting pre actor: %w", err)
	}
	curAct, err := ca.api.StateGetActor(ctx, actor, cur)
	if err != nil {
		return nil, fmt.Errorf("getting cur actor: %w", err)
	}

	preSt, err := miner.Load(store, preAct)
	if err != nil {
		return nil, fmt.Errorf("loading miner actor: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("diff precommits: %w", err)
	}
